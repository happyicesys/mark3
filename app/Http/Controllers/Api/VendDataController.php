...
class VendDataController extends Controller
{
    ...
    private function createVendTransaction(Vend $vend, $request)
    {
        if($payType = $request->PAY_TYPE) {
            $payMethod = PayMethod::where('code', $payType)->first();
        }
        ...
        $vendTransaction = VendTransaction::create([
            ...
            'pay_method_id' => isset($payMethod) ? $payMethod->id : 0,
            'vend_id' => $vend->id,
        ...
        if($vendChannelError) {
            $this->syncVendChannelErrorLog($vend, $request->SId, $request->sErr, $vendTransaction->id);
        }
    }
